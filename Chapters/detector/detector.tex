...
\begin{document}
    ...
    precision and rare decay measurements aswell as searches for forbiden processes and exotic particles.  
    ...
    few years. The experimental setup was desinged to fufill all the requirements for the challenging meaurement of $K^+ \rightarrow \pi^{+} \nu \bar{\nu}$, which involves the 
    ...
    The measuremnt of the missing mass is used for kinemtic rejection of the main background $K^{+}$ decays; $K^+ \rightarrow \pi^{+} \pi^{0}$, $K^+ \rightarrow \pi^{+} \pi^{-} + \pi^{0}$ and $K^+ \rightarrow \mu^{+} \nu_{\mu}$ by selecting two regions shown in figure \ref{fig:missing-mass-signal}.
    The missing mass calculation requires precise timing and poistion measurement of the $K^{+}$ and $\pi^{+}$ candidates, provided by beam (section \ref{sec:GTK}) and secondary (section \ref{sec:STRAW}) particle spectrometers. 
    The measured 3-momenta of the candiates alows for the reconstruction of the 4-momenta of the Kaon $P_{K^{+}}$ and the pion $P_{\pi^{+}}$ which is used in equation \ref{eq:missingmass}
    ...
    \end{figure}
    This rest of this chapter will cover the how NA62 fufils the requirements layed out above. Section \ref{sec:BeamLine} will describe the NA62 beam-line, upstream of the NA62 detector.
    The NA62 detector comprises of a number of sub-detectirs which will be described in Section \ref{sec:sub-det}, with the trigger and data acquisition system (TDAQ) following in Section \ref{sec:TDAQ} 
    ...
    \begin{figure}[H]
        ...
        \label{fig:na62-det}
    \end{figure}
    \section{Beam-Line}\label{sec:BeamLine}
    The NA62 experiment uses a beam extracted from the SPS with a nominal momentum of 400\,$GeV/c$ which is directed onto a 400\,mm long, 2\,mm diameter berylium rod that forms
    the fixed target (T10) located at 0\,m in figure \ref{fig:beam-line}. The beam derived from the T10 target is a high-intensity hadron beam refered to as K12 has a momentum of
    75\,$GeV/c$, which is chosen to maximise the fraction of kaons with respect to the other beam components. 
    \newline\indent The first elements of the K12 beam-line are three quadripiole magnets (Q1, Q2 and Q3), which are used to focus the beam with a large solid angle acceptance of $\pm2.7$\,mrad 
    horizontally and $\pm1.5$\,mrad vertically at a 75\,$GeV/c$ central momentum. Next follows an acromat (A1), which selects a beam of a 75\,$GeV/c$ momentum with a 1\,\% rms momentum bite. 
    The A1 acromat comprises of four dipole magnets which vertically deflect the beam, the first two which deflect the beam 110\,mm downwards, while the final two deflet the beam upwards towards
    the orignal axis. Between these pairs of magnets there are a pair of water cooled beam-dump units (TAX1 and TAX2) with a set of holes to select the required 75\,$GeV/c$ momentum beam, while  
    absorbing unwanted secondary beam particles and remaining primary protons. TAX1\&2 can be closed to act as a secondary target or for saftey when access is required further upstream on K12 beam-line.
    %The beam used in NA62 is produced from a 400\,GeV proton beam from the Super Proton Synchrotron (SPS) incident on a Beryllium target (T10), resulting in a secondary beam containing charged kaons. 
    %The secondary beam produced has a momentum of 75\,GeV and has a composition of mostly pions (70\,\%), protons (23\,\%), kaons (6\,\%) and a small number of muons ($<$1\,\%). 
    \begin{figure}[H]
        \centering
        \includegraphics[width=13cm]{imgs/BeamLine.png}
        \caption{Schematic of K12 beamline from T10 target at 0m to the entence of the NA62 decay region at 102.4m\cite{Gil_2017}.}
        \label{fig:beam-line}
    \end{figure}
    \section{Sub Detectors}\label{sec:sub-det}
    \subsection{KTAG}
    \subsection{GigaTracker (GTK)}\label{sec:GTK}
    \subsection{Straw Spectrometer (Straw)}\label{sec:STRAW}
    \subsection{RICH}
    \subsection{Photon Detection System}
    \section{Triger and Data Acquisition Sytem (TDAQ)}\label{sec:TDAQ}
